...
- Provider-account activation, ENS name registration/writes, and production deployment until their separately authorized integration tasks.
- Foundry, The Graph, authentication, wallets for ordinary exploration, and all P1/P2 features.

## Phase 1 exit gate

The foundation checkpoint is complete when local quality commands pass and the Phaser shell reaches its configured Convex development deployment. Sponsor acceptance, GitHub publishing, and Vercel preview deployment are later integration gates under the core-first sequencing decision. Each checkpoint still requires diff review and human approval before commit.
